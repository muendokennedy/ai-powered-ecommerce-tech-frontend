    ...
    <div class="flex-1 flex flex-col">
      ...
      <main class="flex-1 overflow-y-auto p-6">
        <div class="max-w-6xl mx-auto">
          ...
          <div v-if="saveMessage" class="mb-6 p-4 rounded-lg bg-green-50 dark:bg-green-900/40 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-700 text-sm font-medium">{{ saveMessage }}</div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
              ...
              <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <div class="relative">
                      <input :value="form.category" disabled class="w-full px-4 py-3 border border-gray-300 dark:border-gray-700 rounded-lg bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-300 cursor-not-allowed" />
              ...
              <!-- Images -->
              <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-6">Images</h2>
                    ...
                    <div @click="triggerFileDialog('edit-primary-image')" @dragover="onDragOver($event, 'primaryImage')" @dragleave="onDragLeave($event, 'primaryImage')" @drop="onDrop($event, 'primaryImage')" :class="['relative group border-2 border-dashed rounded-lg text-center transition-colors cursor-pointer', dragOver.primaryImage ? 'border-[#042EFF] bg-[#042EFF]/5' : 'border-gray-300 hover:border-[#042EFF]', imagePreviews.primaryImage ? 'p-2' : 'p-6']">
                      ...
                      <template v-else>
                        ...
                        <div class="mt-2">
                          ...
                          <span class="block text-xs text-gray-500 dark:text-gray-400">Primary (Hero)</span>
                        </div>
                      </template>
                  ...
                  <div>
                    ...
                    <div @click="triggerFileDialog('edit-secondary-image')" @dragover="onDragOver($event, 'secondaryImage')" @dragleave="onDragLeave($event, 'secondaryImage')" @drop="onDrop($event, 'secondaryImage')" :class="['relative group border-2 border-dashed rounded-lg text-center transition-colors cursor-pointer', dragOver.secondaryImage ? 'border-[#042EFF] bg-[#042EFF]/5' : 'border-gray-300 hover:border-[#042EFF]', imagePreviews.secondaryImage ? 'p-2' : 'p-6']">
                      ...
                      <template v-else>
                        ...
                        <div class="mt-2">
                          <span class="block text-xs font-medium text-gray-900 dark:text-gray-100">Drag & Drop or Click</span>
                          <span class="block text-xs text-gray-500 dark:text-gray-400">Secondary (Angle)</span>
                        </div>
                      </template>
                  ...
                  <div>
                    ...
                    <div @click="triggerFileDialog('edit-tertiary-image')" @dragover="onDragOver($event, 'tertiaryImage')" @dragleave="onDragLeave($event, 'tertiaryImage')" @drop="onDrop($event, 'tertiaryImage')" :class="['relative group border-2 border-dashed rounded-lg text-center transition-colors cursor-pointer', dragOver.tertiaryImage ? 'border-[#042EFF] bg-[#042EFF]/5' : 'border-gray-300 hover:border-[#042EFF]', imagePreviews.tertiaryImage ? 'p-2' : 'p-6']">
                      ...
                      <template v-else>
                        ...
                        <div class="mt-2">
                          <span class="block text-xs font-medium text-gray-900 dark:text-gray-100">Drag & Drop or Click</span>
                          <span class="block text-xs text-gray-500 dark:text-gray-400">Tertiary (Detail)</span>
                        </div>
                      </template>
            ...
            <div class="space-y-8">
              <!-- Specifications -->
              <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4 flex items-center">Specifications
                  <span class="ml-2 text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500 font-medium">(Edit Inline)</span>
                </h2>
                <div v-if="specEntries.length" class="space-y-4">
                  <div v-for="([key]) in specEntries" :key="key" class="grid grid-cols-5 gap-3 items-center group">
                    <div class="col-span-2 text-xs font-medium text-gray-600 dark:text-gray-300 uppercase tracking-wide">
                      {{ key.replace(/([A-Z])/g,' $1').trim() }}
                    </div>
                    <div class="col-span-3">
                      <input v-model="form.specifications[key]" type="text" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 rounded-md focus:ring-[#042EFF] focus:border-[#042EFF] text-sm" />
                    </div>
                  </div>
                </div>
                <div v-else class="text-xs text-gray-500 dark:text-gray-400">No specifications present.</div>
              </div>

              <!-- Metadata / Quick Info -->
              <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Meta</h2>
                <div class="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p class="font-medium text-gray-900 dark:text-gray-100">{{ originalProduct?.sku || '—' }}</p>
                  </div>
                  <div>
                    <p class="text-gray-500 dark:text-gray-400">Category</p>
                    <p class="font-medium text-gray-900 dark:text-gray-100">{{ form.category }}</p>
                  </div>
                  <div>
                    <p class="text-gray-500 dark:text-gray-400">Status</p>
                    <p class="font-medium text-gray-900 dark:text-gray-100">{{ originalProduct?.status || '—' }}</p>
                  </div>
                </div>
              </div>

              <!-- Danger / Actions -->
              <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-red-100 dark:border-red-900/40 p-6">
